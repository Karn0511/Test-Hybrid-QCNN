...
# Compatibility shim: newer transformers may expect register_pytree_node,
# while torch 2.1 exposes only _register_pytree_node.
...
def _cached_encode(texts: list[str], model_name: str, batch_size: int = 128) -> np.ndarray:
    """
    v38.6 Xeon-Sentinel: Optimized BERT Core.
    Abolishes multi-process encoding on Windows to prevent CPU idling/hangs.
    Saturates the 32-core Xeon via high-threading (Single-Process).
    """
    import torch
    import os
    from sentence_transformers import SentenceTransformer
    
    # 1. Threading Optimization for Xeon Gold
    cpu_count = os.cpu_count() or 1
    torch.set_num_threads(cpu_count)
    logger.info(f"[XEON-OPT]: Thread-pinning locked to {cpu_count} logical cores.")
    
    try:
        model = SentenceTransformer(model_name, local_files_only=True)
    except:
        logger.warning("[!] Local model not found. Downloading...")
        model = SentenceTransformer(model_name, local_files_only=False)
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # 2. Sequential Strike (Reliable & High-Throughput)
    # 10,000 samples on a Xeon Gold 5218 with 32 threads is very fast.
    logger.info(f"[CORE-IGNITION]: Encoding {len(texts)} samples on {device.upper()}...")
    emb = model.encode(
        texts, 
    ...
    logger.info("[SUCCESS]: Semantic manifold encoded. Locking to disk cache.")
    return emb
